...
public class Gonggao extends Activity implements OnFooterRefreshListener,OnHeaderRefreshListener{
	PullToRefreshView mPullToRefreshView;
	public static ListView lv;
	Listviewgonggaoadapter adapter;
	...
	private EditText ed;
	private String[] name = {"我啊你你","这是什么","徐阔","好的","上大街上到处","上班粉红色","呵呵呵"};
	@Override
	...
	private void initview() {
		...
		lly = (LinearLayout) findViewById(R.id.gonggao_layout);
		...
		shuaxin = (LinearLayout) findViewById(R.id.shuaxinyemian_gonggao);
		...
		ed = (EditText) findViewById(R.id.gonggao_sousuo);
		...
		lv = (ListView) findViewById(R.id.gonggaoListView);
		adapter = new Listviewgonggaoadapter(getApplicationContext());//给适配器初始化
		lv.setAdapter(adapter);
		ImageButton ib = (ImageButton) findViewById(R.id.xinjianbangong);
...
//					startActivity(intent);
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position,long id) {
				...
				intent.setClass(getApplicationContext(), GonggaoParticulars.class);
				startActivity(intent);
			}
		});
	}
...
//				mPullToRefreshView.onFooterRefreshComplete();
	...
	private void initPopWindow(){  
		...
		popupWindow = new PopupWindow(findViewById(R.id.gonggao_layout), 500, 900);//设置popwindow的宽度和高度
	...
	public void getgonggaolist(){
		...
		request.PostOA("get_notice_list", params, new Handler(){
			@Override
			// 当有消息发送出来的时候就执行Handler的这个方法
			public void handleMessage(Message msg){
				super.handleMessage(msg);
				String strResult = msg.obj.toString();
				System.out.println("看看这个公告里面都是什么"+strResult);
				int status_code = 200;
				try {
					...
					case -1:
						...
						if(Listviewgonggaoadapter.title.size()==0){
				...
				try {
					...
					Listviewgonggaoadapter.title.clear();
					Listviewgonggaoadapter.time.clear();
					Listviewgonggaoadapter.content.clear();
					Listviewgonggaoadapter.colorarr.clear();
					Listviewgonggaoadapter.id.clear();
					for (int i = 0; i < jsonlxt.length(); i++) {
						json = jsonlxt.getJSONObject(i);
						Listviewgonggaoadapter.title.add(json.getString("Title"));
						Listviewgonggaoadapter.time.add(json.getString("PublishTime"));
						Listviewgonggaoadapter.content.add(json.getString("Summary"));
						Listviewgonggaoadapter.colorarr.add(json.getString("Color"));
						Listviewgonggaoadapter.id.add(json.getString("id"));
					}
					//加载数据结束时提示框消失
					if(Listviewgonggaoadapter.title.size()>0){
						progressdialog.dismiss();
